    ...
    async def _scan() -> Dict[str, Any]:
        ...
        async with streamablehttp_client(
            ...
            async with ClientSession(read, write) as session:
                ...
                logger.info("MCP tool call: get_upload_url with args %s", upload_args)
                upload_result = _parse_tool_result(
                    await session.call_tool("get_upload_url", arguments=upload_args)
                )
                logger.info("MCP tool response: %s", upload_result)
        ...
        async with streamablehttp_client(
            ...
            async with ClientSession(read2, write2) as session2:
                ...
                logger.info("MCP tool call: analyze_uploaded_archive with args %s", analyze_args)
                result = _parse_tool_result(
                    await session2.call_tool("analyze_uploaded_archive", arguments=analyze_args)
                )
                logger.info("MCP tool response: %s", result)
                return result
...
def apply_pipeline_fix_code_to_clone(
    ...
    for filepath, actions in by_file.items():
        ...
        if patch_applied and content != original_content:
            # Validate for dangerous code patterns
            if re.search(r'\b(eval|exec|system|popen|subprocess\.run)\s*\(', content):
                logger.warning("Dangerous code patterns detected in %s, fix skipped", rel_path)
